  ...
  var Decimal = /*#__PURE__*/function () {
    ...
    _createClass(Decimal, [{
      ...
      value: function toExponential(places) {
        var t,es,absm;
        if (this.layer === 0) {
          return (this.sign * this.mag).toExponential(places).replace('+','');
        }
        if (this.mag < 0) {
          if (this.layer === 1) {
            es = this.e+'';
            if (es.length - 1 <= places) {
              return (this.sign === -1 ? "-" : "") + this.m.toFixed(places - (es.length - 1)) + 'e' + es;
            }
            if (es.length - 1 === places+1) {
              return (this.sign === -1 ? "-" : "") + 'e' + FC(-1, this.layer - 1, -this.mag).toFixed(0);
            }
          }
          return (this.sign === -1 ? "-" : "") + 'e' + FC(-1, this.layer - 1, -this.mag).toExponential(places);
        }
        t = new Decimal(this);
        t.layer = 1;
        if ((es = t.e+'').length <= places){
          absm = Math.abs(t.m)
          return (this.sign === -1 ? "-" : "") + (this.layer-1 <= MAX_ES_IN_A_ROW ? "e".repeat(this.layer-1) : "(e^" + (this.layer-1) + ")") + (absm >= 10-0.5*10**(es.length - places) ? (1).toFixed(places - es.length) + 'e' + (t.e+1) : absm.toFixed(places - es.length) + 'e' + es);
        } else {
          return (this.sign === -1 ? "-" : "") + (this.layer <= MAX_ES_IN_A_ROW ? "e".repeat(this.layer) : "(e^" + this.layer + ")") + (es.length === places+1 ? this.mag.toFixed(0) : this.mag.toExponential(places).replace('+',''));
        }
      }
    }, {
      key: "toFixed",
      ...
      value: function toPrecision(places) {
        ...
        if (places > e && -places < e) {
          return (+this).toPrecision(places).replace('+','');
        }
        return this.toExponential(places - 1);
      }
    }, {
